...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout", type='secondary', key='loginbackbtn',width='stretch'):
...
def teacher_screen_login():
    ...
    with c1:
        header_dashboard()
    with c2:
        if st.button("Go back to Home", type='secondary', key='loginbackbtn',width='stretch'):
            st.session_state['login_type'] = None
            st.rerun()

    st.header('Login Page',text_alignment='center')
    st.space()
    st.space()
...
def teacher_screen_register():
    ...
    teacher_name = st.text_input("Full name", placeholder='📝 Full Name')

    teacher_pass = st.text_input("Create password", type='password', placeholder="🔒 Password")
